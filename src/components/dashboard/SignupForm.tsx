...
import { signUp, updateUser } from "@/_actions/userActions"
...
export function SignUpForm({ type, userData }: Props) {
    ...
    async function signUpAction(data: SignUpFormSchema) {
        ...
        try {
            ...
            if (type === "edit") {
                res = await updateUser(result.data)
            } else {
                res = await signUp(result.data)
            }
            if (!res?.error && res?.status === 201 || res?.status === 200) {
                toast({
                    title: "مرحبا بك",
                    ...
                    <div className="flex flex-col justify-start p-4 space-y-4 rounded-md shadow-lg ">
                        <form
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>كلمة السر القديمة</FormLabel>
                                        <FormControl>
                                            <div className="relative">
                                                <Input placeholder="*******" {...field} type={`${showPassword.oldPassword ? "text" : "password"}`} />
                                                {showPassword.oldPassword ?
                                                    <PiEyeBold
                                                        className={`hover:cursor-pointer absolute left-[10%] bottom-[28%]`}
                                                        onClick={() => setShowPassword(prevState => ({ ...prevState, oldPassword: false }))} /> :
                                                    <PiEyeClosedBold
                                                        className={`hover:cursor-pointer absolute left-[10%] bottom-[28%]`} onClick={() => setShowPassword(prevState => ({ ...prevState, oldPassword: true }))} />
                                                }
                                            </div>
                                        </FormControl>
                                        <FormMessage />
