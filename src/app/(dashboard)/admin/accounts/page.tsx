...
import UsersTable from "@/components/dashboard/UsersOptions"
...
export default async function UsersForm({ searchParams}: Props) {
  const userId = searchParams?.id
  const users: User[] = await getUsers()
  const posts: Post[] = await getPosts()
  const user: User = users.find((user) => user.id === Number(userId))!
  ...
  return (
    <section className="container flex flex-col items-center justify-center gap-8 my-20 ">
      {sessionUser?.role === "SuperAdmin" ? <SignUpForm /> : null}
        <UsersTable userId={parseInt(userId!)} users={users} posts={posts} user={user}/>
